...
with col1:
    ...
    # else:
    #     if not st.session_state.vector_store: # Show only if no PDF is ever loaded
    #         st.info("Upload a PDF to begin your research.")
...
with col2:
    ...
    with chat_container:
        for i, message in enumerate(st.session_state.chat_history_display):
            with st.chat_message(message["role"], avatar="🧑‍🎓" if message["role"] == "user" else "🔬"): # Academic avatars
    ...
    if prompt := st.chat_input(
        ...
        with chat_container: # Process and display assistant's response
            with st.chat_message("assistant", avatar="🤖"):
                ...
                # Pass the existing conversation_object if it exists, otherwise it will be created
        ...
        # st.rerun() # Consider if needed for complex UI updates

if not uploaded_file and not st.session_state.chat_history_display:
    with col2: st.info("☝️ Welcome! Please upload a research paper (PDF) to begin your analysis.")
elif not GEMINI_API_KEY:
    with col2: st.error("🚫 Please enter your Google Gemini API Key in the sidebar to enable interaction.")
elif uploaded_file and not st.session_state.vector_store:
     with col2: st.warning("⏳ PDF processing might have failed or is still in progress. Check for errors or try re-uploading.")